...
rho_list_diffz=list(rep(0,5),
                    c(rhox,  0),
                    c(rhox,  -0.3),c(rhox,   0.3))

xi=list(xi_main=c(0.4,-0.2,0.3,-0.2),
        xi_x_cross=c(0.2,-0.2,0.1),
        xi_zx_cross=c(0.2,0,-0.2))
...
res_single<-main_simu2_parallel(nco=9,N=1000,nt=nt,ns=ns,betat=betat,betas=betas,xi=xi,
                                zparameter = zparam)
t2=Sys.time()
t2-t1
...
for(i in 2:length(rho_list_diffz) ){
...
save(reslist,rho_list_diffz,zparam,evalua,betat,auc_true_est,file="res/summary_navie_ci_ns500_DM.rdata")
