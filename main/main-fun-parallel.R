...
main_simu2_parallel<-function(nco,N=100,nt,ns,betat,betas,xi,
                     zparameter=list(beta= c(0.4,0.5,-0.2,0.3),
                                     sigma_z=0.7, include_interaction=FALSE)){
  ...
  # 导出必要对象到 workers
  # 注意：data_list 是局部变量，foreach 会自动捕获，但为了清晰也可显式导出
  clusterExport(cl, 
                varlist = c("data", "p", "q", "hb", "ns", "nt"),
                envir = environment())
  
  # 在每个 worker 中加载所需包和自定义函数
  clusterEvalQ(cl, {
    Sys.setenv(
      OMP_NUM_THREADS = "1",
    ...
    # 如果 est_full_simple, middle_f2_simple 等函数也在 fun_boot.R 中，则已加载
    NULL
  })
...
main_simu2_parallel_test<-function(nco,N=100,nt,ns,betat,betas,xi,
                              zparameter=list(beta= c(0.4,0.5,-0.2,0.3),
                                              sigma_z=0.7, include_interaction=FALSE)){
